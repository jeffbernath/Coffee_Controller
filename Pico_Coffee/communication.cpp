...
void handle_command(char *line)
{
    ...
    if (std::strcmp(verb, "SET") == 0) {
        ...
        char error[64] = {0};
        if (!io_runtime_set(name, value, error, sizeof(error))) {
    ...
    if (std::strcmp(verb, "CMD") == 0) {
        ...
        if (std::strcmp(command, "TARE") == 0) {
            char error[64] = {0};
            if (!io_runtime_tare_all(error, sizeof(error))) {
                send_error(sequence, target ? target : "TARE", error);
                return;
            }
            send_ack(sequence);
            return;
        ...
        send_error(sequence, command, "UNKNOWN_COMMAND");
        return;
    }
}
...
void communication_poll()
{
    ...
    while ((ch = getchar_timeout_us(0)) != PICO_ERROR_TIMEOUT) {
        ...
        if (g_rx_length < sizeof(g_rx_buffer) - 1) {
            g_rx_buffer[g_rx_length++] = static_cast<char>(ch);
        } else {
            g_rx_length = 0;
        }
    }
    ...
    if (now_ms - g_last_heartbeat_ms >= IO_HEARTBEAT_MS) {
...
{
    std::printf("HELLO,%lu,0.2.0\n", static_cast<unsigned long>(IO_PROTOCOL_VERSION));
}
...
void communication_send_io(const IoDefinition &definition, float value)
{
    if (definition.data_type && std::strcmp(definition.data_type, "bool") == 0) {
        std::printf("IO,%s,%d\n", definition.name, value >= 0.5f ? 1 : 0);
    } else {
        std::printf("IO,%s,%.3f\n", definition.name, static_cast<double>(value));
    }
}

void communication_send_io_unavailable(const IoDefinition &definition)
{
    std::printf("IO_STATUS,%s,UNAVAILABLE\n", definition.name);
}
